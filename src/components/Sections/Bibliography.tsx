 ...
 */
function isCited( refName: string ): boolean {
 ...
 */
function generateTag( refName: string ): number {
 ...
 */
export function Cite(name: string) {
  let referenceTag: string;
  if (tags[name] === undefined) {
    // generate new key
    referenceTag = '[{}]'.replace('{}', String(generateTag(name)));
    tags[name] = referenceTag;
  } else {
    // key already defined in tag dict, get it
    referenceTag = tags[name];
  }
  console.log(references);
  console.log(tags);
  
  return <a href={'#'.concat(name)}>{referenceTag}</a>
}
